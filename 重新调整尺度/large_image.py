...
#     assert len(det_data_samples) == len(offsets), 'The `results` should have the same length with `offsets`.'
#     shifted_predictions = []
...
#         if hasattr(pred_inst, 'masks') and pred_inst.masks is not None:
#             # Ensure masks are numpy arrays before shifting
#             numpy_masks = [mask.cpu().numpy() for mask in pred_inst.masks]
#             shifted_masks = shift_masks(numpy_masks, offset, src_image_shape)
#             pred_inst.masks = [torch.from_numpy(mask) for mask in shifted_masks]
...
def merge_results_by_nms(results: SampleList, offsets: Sequence[Tuple[int,
    ...
    # merged_result = results[0].clone()
    # merged_result.pred_instances = merged_instances
    return merged_instances # merged_result
